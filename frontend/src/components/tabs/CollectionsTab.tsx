...
import { 
  ...
  X,
  Briefcase,
  ArrowLeft // ADDED: Icon for the back button
} from "lucide-react";
...
export default function CollectionsTab() {
  ...
  const handleDelete = async (id: string) => {
    if (!confirm("⚠️ Are you sure you want to delete this collection?")) return;
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 bg-gray-50 min-h-screen">
      ...
      <div className="mb-6">
        <button
          onClick={() => navigate(-1)}
          className="flex items-center gap-2 px-4 py-2 bg-white border rounded-lg text-gray-700 hover:bg-gray-100 transition shadow-sm"
        >
          <ArrowLeft size={18} />
          Back
        </button>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50 p-4">
          <div className="bg-white p-6 rounded-xl shadow-xl w-full max-w-2xl animate-fadeIn relative">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <div className="border rounded-lg p-2 max-h-48 overflow-y-auto space-y-1">
                    ...
                    )) : <p className="p-4 text-sm text-center text-gray-500">No students found.</p>}
                  </div>
                </div>
                {/* Teacher Selection */}
                <div>
                    ...
                    <div className="border rounded-lg p-2 max-h-48 overflow-y-auto space-y-1">
                        ...
                        )) : <p className="p-4 text-sm text-center text-gray-500">No teachers found.</p>}
                    </div>
                </div>
            ...
            <div className="flex justify-end gap-3 mt-6">
              <button onClick={closeModal} className="px-4 py-2 bg-gray-200 text-gray-800 rounded-lg hover:bg-gray-300 transition">Cancel</button>
              <button onClick={handleSubmit} className="px-4 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 transition">{editModal ? "Save Changes" : "Create"}</button>
            </div>
          </div>
        </div>
      )}
